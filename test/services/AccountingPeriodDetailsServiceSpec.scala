...
import connectors.AccountingPeriodDetailsConnector
...
class AccountingPeriodDetailsServiceSpec
  ...
  private trait BaseSetup {
    ...
    val mockConnector: AccountingPeriodDetailsConnector = mock[AccountingPeriodDetailsConnector]
    val service                                         = new AccountingPeriodDetailsService(mockConnector)
    val taxPayerReference: Long                         = 12L
    val accPeriod: Long                                 = 2L
  }

  "AccountingPeriodDetailsService.getAccountingPeriodResponse" should {
